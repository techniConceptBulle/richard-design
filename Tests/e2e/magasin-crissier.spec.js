...
test.describe("Magasin Crissier page", () => {
  ...
  test("renders selected products as a slider", async ({ page }) => {
    ...
    await expect(slides.nth(0)).toHaveAttribute("href", "/pages/category.html?slug=lit");
    await expect(slides.nth(1)).toHaveAttribute("href", "/pages/category.html?slug=matelas");
    await expect(slides.nth(2)).toHaveAttribute("href", "/pages/category.html?slug=sommier");
    await expect(slides.nth(3)).toHaveAttribute("href", "/pages/category.html?slug=duvets");
    await expect(slides.nth(4)).toHaveAttribute("href", "/pages/category.html?slug=oreillers");
